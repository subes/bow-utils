...
public class CachesManager implements MemoryGobbler, StatusViewable, CloseableComponent {
    ...
    public synchronized <T> Cache<T> createNewCache(CacheableData<T> cacheableData, boolean isWriteBuffer, String name) {
        Cache newCache = new Cache<T>(cacheableData, isWriteBuffer, name);
        caches.add(newCache);
        return newCache;
    }
