...
public class Long2IntOpenHashMap extends AbstractLong2IntMap implements java.io.Serializable, Cloneable, Hash {
    ...
    public Integer put(final long k, final Integer ov) {
        final int v = ov;
        // The starting point.
    ...
    public Integer get(final long k) {
        // The starting point.
        int pos = (int) it.unimi.dsi.fastutil.HashCommon.murmurHash3((k) ^ mask) & mask;
        // There's always an unused entry.
        while (used[pos]) {
    ...
    public boolean containsValue(final Integer integer) {
        int v = (Integer) integer;
     ...
     * the map
     */
    private final class MapEntry implements Entry<Integer> {
    ...
    private class MapIterator {
        ...
        {
            final boolean used[] = Long2IntOpenHashMap.this.used;
        ...
        public void remove() {
            ...
            if (pos < -1) {
                ...
                Long2IntOpenHashMap.this.remove(wrapped.getLong(-pos - 2));
                last = -1;
                return;
            }
            size--;
